...
expr *emit_tablecreate_indexed_with_stack(expr *entry);
...
void emit_tablecreate_indexed_with_stack2(void);
extern int lines;
extern char *yytext;
extern FILE* yyin;
extern FILE* yyout;
extern int yylineno;
char *print_ex_via_type(expr *ex);
...
int nestedObj = 0;
unsigned table_flag = 0;
int nestedObj_counter = 0;
bool isEmitted = 0;
char err[90];
%}
...
member          :   lvalue DOT ID
						{
							char s[50];
							sprintf(s, "member -> lvalue.ID %s\n", yylval.token -> content);
							write_rule(s);
		...
		|	LBRA indexed RBRA
				{
					...
					$$ = emit_tablecreate_indexed_with_stack((expr *)0);
...
indexed 	:	indexedelem indexed2
					{
						char s[100];
						sprintf(s,"indexed -> ,indexedelem indexed2 \n");
						write_rule(s); 
						
						push_pair(indexed_stack, $1);
						$$ = $1;
...
indexed2	:	COMMA indexedelem indexed2 
					{
						char s[100];
						sprintf(s,"indexed2 -> ,indexedelem indexed2 \n" );
						write_rule(s);
						
						push_pair(indexed_stack, $2);
						$$ = $2;
					}
...
indexedelem 	: 	LCURBRA expr COLON expr RCURBRA
					{
							char s[100];
							sprintf(s,"0indexedelem -> {expr:expr}\n" );
							write_rule(s);
...
forstmt        :    forprefix N elist RPAREN N {empty_stack(elist_stack);} loopstmt N    
						{
							write_rule("forstmt -> FOR(elist;expr;elist;)stmt\n");
							push(continuejumpstack, $2+1);
							manage_for_emition($1,$2,$5,$7,$8);
							//patch labels
							patch_loopend_labels(num_of_breaks,num_of_continues);
							//and pop to retrieve the previous ones.
							num_of_breaks = pop(breakcounterstack)->item;
...
%%
char *
print_ex_via_type(expr *ex) {
	switch(ex -> type) {
		case var_e : return ex -> sym->value.varVal -> name;
		case conststring_e: return ex -> strConst;
		case constint_e: {
			int length = snprintf( NULL, 0, "%d", ex -> intConst);
			char *s = (char *)malloc(sizeof(char)*(length)+1); 
			sprintf(s,"%d", ex -> intConst); 
			return s;
		}
...
manage_logical_op(char *type, expr *ex1, int M, expr *ex2) {
	...
	if(ex2 -> type != boolexpr_e) {
		...
		if(!strcmp(type , "and")) {
			...
			merge_tflists(ex1 -> falselist, temp -> falselist);
			term -> falselist = ex1 -> falselist;
		} else if(!strcmp(type,"or")) {
			backpatch(ex1 -> falselist, M);
			merge_tflists(ex1 -> truelist, temp -> truelist);
			term -> truelist = ex1 -> truelist;
		...
		if(!strcmp(type , "and")) {
			...
			merge_tflists(ex1 -> falselist, ex2 -> falselist);
			term -> falselist = ex1 -> falselist;
		} else if(!strcmp(type,"or")) {
			backpatch(ex1 -> falselist, M);
			merge_tflists(ex1 -> truelist, ex2 -> truelist);
			term -> truelist = ex1 -> truelist;
...
manage_assign_emition(expr* lval,expr * exp)
{
	...
	if(exp -> type == boolexpr_e) {
		...
		if(lval->type == tableitem_e)
		{
			...
			emit(tablesetelem,lval->index,meriki,lval,0,yylineno);
...
manage_ex_op_ex_emit(expr* expr1,struct alpha_token_t* op,expr* expr2)
{
	...
	else if(type == boolexpr_e)
	{
		if(expr1 -> type == boolexpr_e) {
			...
			backpatch(expr1 -> truelist, nextquad());
			backpatch(expr1 -> falselist, nextquad()+2);
		...
		if(expr2 -> type == boolexpr_e) {
			...
			backpatch(expr2 -> truelist, nextquad());
			backpatch(expr2 -> falselist, nextquad()+2);
...
manage_term_emition(char *type,expr* arg)
{
	...
	else if(!strcmp(type,"n_ex"))
	{
		...
		if(arg -> type != boolexpr_e) {
			...
			if(arg -> truelist != NULL) {
				merge_tflists(arg ->truelist, term-> truelist);
				term -> truelist = arg -> truelist;
				merge_tflists(arg -> falselist, term -> falselist);
				term -> falselist = arg-> falselist;
			}
...
func_start_emit(SymbolTableEntry *func_prefix)
{
	...
	emit(funcstart,NULL,NULL,lvalue_expr(func_prefix),0,yylineno);//or (enum iopcode) funcstart
...
func_end_emit(SymbolTableEntry *func_prefix,unsigned int body)
{
	...
	restorereturnoffset();
	emit(funcend,NULL,NULL,lvalue_expr(func_prefix),0,yylineno);//make lvalue funprefix?
...
void
emit_tablecreate_indexed_with_stack2(void) {
	expr *t  = newexpr(newtable_e);
	t -> sym = newtemp(scope, LOC, yylineno);
	emit(tablecreate, NULL, NULL, t,0,yylineno);
	while(!ispairEmpty(indexed_stack)) {
		pair *ind = pop_pair(indexed_stack);
		pair_expr *i = ind -> item ;//-> value;
		printf("creating index [%s,]\n", print_ex_via_type(i -> index));
		emit(tablesetelem, i -> index, i->value, t,0,yylineno);
		
	}
	empty_stack_pair(indexed_stack);
	assert(ispairEmpty(indexed_stack));
}

expr *
emit_tablecreate_indexed_with_stack(expr *entry) {
	expr *t  = newexpr(newtable_e);
	t -> sym = newtemp(scope, LOC, yylineno);
	emit(tablecreate, NULL, NULL, t,0,yylineno);
	while(!ispairEmpty(indexed_stack)) {
...
// void
// emit_pairlist(pair_expr *indexed) {
// 	expr *t = newexpr(newtable_e);
// 	t -> sym = newtemp(scope,LOC,yylineno);
// 	while()
// }
...
make_call(expr *lv, expr *reversed_list) {
	expr *meriki_list, *meriki_tail, *iterator;

	meriki_list = meriki_tail = NULL;
	iterator = reversed_list;
	while(iterator != NULL) {
		
		if(iterator -> type == boolexpr_e) {
			// Meriki apotimisi
			expr *meriki  = newexpr(boolexpr_e);
			meriki -> sym = newtemp(scope, LOC, yylineno);
			
			backpatch(iterator -> truelist, nextquad());
			backpatch(iterator -> falselist, nextquad()+2);
			...
			//emit(param, meriki,NULL, NULL,0,yylineno);
			if(meriki_list == NULL) {
				meriki_list = meriki;
				meriki_tail = meriki;
			} else {
				meriki_tail -> next = meriki;
				meriki_tail = meriki;
			}
			meriki -> next = NULL;
		} else {
	...
	while(reversed_list != NULL) {
		if(reversed_list -> type == boolexpr_e) {
			assert(meriki_list);
			emit(param, meriki_list,NULL, NULL,0,yylineno);
			meriki_list = meriki_list -> next;
		} else {
			emit(param, reversed_list,NULL, NULL,0,yylineno);
		}
...
member_item(expr *lv, char *name) {
	...
	ti -> index -> sym = newtemp(scope, LOC, yylineno);
	return ti;
}
...
handle_insertion ( char *name, unsigned int line, char *type ) {
	...
	if(!FUNC(entry))updatecurrspaceoffset();
...
bool init() {
	...
	indexed_stack			= create_stack_pair();
	...
	quads = (quad *)malloc(sizeof(quad)*total);
	for(counter = 0; counter < total; counter++) {
		allocate_quad(quads+counter);
	}

	//  instructions = (instruction *)calloc(total,sizeof(instructions));
	instructions = (instruction *)malloc(sizeof(instruction)*total);
	make_stmt(stmtslist);
	return flag;
}
...
void
remove_previous() {
	if( access( "rules.txt", F_OK ) == 0 ) {
        int del = remove("rules.txt");
        if (!del)
            printf("The file rules.txt was Deleted successfully\n");
        else
            printf("The file rules.txt was not Deleted\n");
    }
}
