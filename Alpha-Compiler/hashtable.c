...
char *
expr_tostring(expr *ex) {
	switch(ex -> type) {
		case conststring_e: return ex -> strConst;
		default: return ex -> sym -> value.varVal->name;
	}
}


void 
push_pair(stack_pair *s, struct pair_expr *it) {
    pair *p = (pair *)malloc(sizeof(pair));
    p -> item = it;
    
	// memcpy(p->item, it, sizeof(pair_expr));
    p -> next = NULL;
...
pair *
pop_pair(stack_pair *s) {
    pair *tail = NULL;

    if(s -> size > 0) {
        tail = s -> items;

        if(s -> items -> next == NULL) {
            s -> items = NULL;
            s -> top = NULL;
        } else {
